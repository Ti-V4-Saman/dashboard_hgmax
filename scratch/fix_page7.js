...
const injectionTarget = `  const [dateRange, setDateRange] = useState({ from: subDays(new Date(), 30), to: new Date() });`;

const newHandler = `  const [dateRange, setDateRange] = useState({ from: subDays(new Date(), 30), to: new Date() });
...
// Now find the exact AdvancedDatePicker tag and replace its inline handler
const datePickerRegex = /<AdvancedDatePicker\s+range=\{dateRange\}\s+onRangeChange=\{.*?\}\s+currentPreset=\{datePreset\}\s*\/>/s;
content = content.replace(datePickerRegex, `<AdvancedDatePicker range={dateRange} onRangeChange={handleRangeChange} currentPreset={datePreset} />`);

fs.writeFileSync('app/page.tsx', content);
console.log('Fixed date presets successfully.');
